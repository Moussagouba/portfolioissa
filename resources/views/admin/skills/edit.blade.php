...
@section('content')
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <h2 class="text-2xl font-bold mb-6">Modifier la Compétence</h2>
        ...
        <form action="{{ route('admin.skills.update', $skill) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="category" id="category" required
                        ...
                        <option value="Développement" {{ old('category', $skill->category) == 'Développement' ? 'selected' : '' }}>Développement</option>
                        <option value="Marketing" {{ old('category', $skill->category) == 'Marketing' ? 'selected' : '' }}>Marketing</option>
                        <option value="Autre" {{ old('category', $skill->category) == 'Autre' ? 'selected' : '' }}>Autre</option>
                    </select>
                    @error('category')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="percentage" id="percentage" min="0" max="100" value="{{ old('percentage', $skill->percentage) }}" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-neonGreen focus:border-transparent">
                    @error('percentage')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="icon" id="icon" value="{{ old('icon', $skill->icon) }}" placeholder="ri-palette-line"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-neonGreen focus:border-transparent">
                ...
                <div>
                    ...
                    <input type="number" name="order" id="order" value="{{ old('order', $skill->order) }}" min="0"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-neonGreen focus:border-transparent">
                    @error('order')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="mt-6">
                ...
                <textarea name="description" id="description" rows="4" required
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-neonGreen focus:border-transparent">{{ old('description', $skill->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
