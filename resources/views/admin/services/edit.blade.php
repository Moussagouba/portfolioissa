        ...
        <form action="{{ route('admin.services.update', $service) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="4" required
                          class="w-full rounded-md border-gray-300 shadow-sm focus:border-neonGreen focus:ring focus:ring-neonGreen focus:ring-opacity-50">{{ old('description', $service->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="image" id="image" value="{{ old('image', $service->image) }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-neonGreen focus:ring focus:ring-neonGreen focus:ring-opacity-50">
                    @error('image')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="price_label" id="price_label" value="{{ old('price_label', $service->price_label) }}" required
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-neonGreen focus:ring focus:ring-neonGreen focus:ring-opacity-50">
            ...
            <div class="mb-6">
                ...
                <div id="features-container">
                    @forelse($service->features as $index => $feature)
                        <div class="mb-2 flex">
                            <input type="text" name="features[]" value="{{ $feature->feature }}" placeholder="Caractéristique"
                            ...
                            <button type="button" class="ml-2 text-red-500 hover:text-red-700 feature-remove {{ $service->features->count() === 1 ? 'hidden' : '' }}">
